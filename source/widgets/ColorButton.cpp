...
ColorToolButton::ColorToolButton(const QIcon& icon, const QString& text) : QToolButton()
{
	...
	connect(mAction, SIGNAL(triggered()), this, SLOT(buttonTriggered()));
}
...
void ColorToolButton::widgetColorChanged(const QColor& color)
{
	setColor(color);
	emit colorChanged(mColor);
}

void ColorToolButton::runColorDialog()
{
	QColorDialog colorDialog(this);
	colorDialog.setOptions(QColorDialog::ShowAlphaChannel);
	colorDialog.setCurrentColor(mColor);
...
void ColorToolButton::buttonTriggered()
{
	emit colorChanged(mColor);
}
...
void ColorToolButton::refreshIcon()
{
	QPixmap pixmap(mIcon.pixmap(size()));
	int barSize = height() / 4;
	...
	if (mColor.alpha() == 0)
	{
		...
		painter.drawRect(1, height() - barSize, height() - 3, barSize - 1);

		painter.setPen(Qt::NoPen);
		painter.setBrush(QColor(170, 170, 170));
		for(int x = barSize + 1; x < height() - 3; x += 2 * barSize - 2)
		{
			if (x + barSize - 1 > height() - 3)
				painter.drawRect(x, height() - barSize, height() - 3 - x - barSize + 1, barSize - 1);
			else
				painter.drawRect(x, height() - barSize, barSize - 1, barSize - 1);
		}
		painter.end();
	}
	else
	{
		QColor fullColor = mColor;
		fullColor.setAlpha(255);

		QPainter painter(&pixmap);
		painter.setPen(QPen(QColor(197, 197, 197), 1));
		painter.setBrush(fullColor);
		painter.drawRect(1, height() - barSize, height() - 3, barSize - 1);
		painter.end();
	}

	mAction->setIcon(QIcon(pixmap));
}

//==================================================================================================

void ColorToolButton::showEvent(QShowEvent* showEvent)
{
	refreshIcon();
	Q_UNUSED(showEvent);
}

void ColorToolButton::resizeEvent(QResizeEvent* resizeEvent)
{
	refreshIcon();
